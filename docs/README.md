...
Latest Tracker / PGP checkpoint (2026-09-14): [current status, v2 review and next steps](TRACKER_PIPELINE_CURRENT_STATUS_2026-09-14.md). The existing zero-call probes favor output fusion, instrument-conditioned verb priors and causal Phase filtering. The current recommendation is an independent M1–M4 research version retaining the old Gate, with Gate v2 refitting as a later cost optimization. **This is a reviewed proposal, not an implemented or promoted v2 release**; the default remains Qwen PGP with Tracker off. Existing result snapshots and audit sources are published, with evaluation limits and unresolved contract issues recorded. No new experiment was run for this synchronization.

PGP full-pipeline integration (2026-09-13): `scripts/run_pipeline.py` now dispatches to the frozen single-Qwen ambiguity-aware Gate. All 6,059 Training cache targets pass end-to-end output/42-feature parity, and 48 related tests pass. Budgeted transport is connected but paid execution was not run; Testing and deployment remain disabled. [Architecture and commands](PGP_PIPELINE_ARCHITECTURE_2026-09-13.md). Previous pipeline and Gate defaults are preserved.

Current PGP research default (2026-09-13, selected after user-authorized assessment): **ambiguity protection + one Qwen probe + whole-frame HGB Gate + exact stopping, Tracker off**, pinned in [DEFAULT_PGP_GATE_VERSION.json](../DEFAULT_PGP_GATE_VERSION.json). Independent refitting reproduced F1 61.698902 / 36,150 errors / corrected 37,157 logical calls. Original outer quality/cost checks pass; outer gain retention is 83.66%, so the 90% retention target **has not passed**. This is only a conditional offline research selection, not independent validation or deployment. [Assessment, costs and limitations](PGP_AMBIGUITY_ASSESSMENT_RESULTS_2026-09-13.md). Prior dual-threshold default and all weights remain preserved. The offline prediction entry verifies selected artifact hashes.

Net-gain PGP trial completed without promotion: [results and diagnosis](PGP_GATE_NET_GAIN_RESULTS_2026-09-13.md). Same F1/errors as the pinned default, 48 fewer logical calls, strict development acceptance failed. Original artifacts and all Tracker files remain unchanged.
