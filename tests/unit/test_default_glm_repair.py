...
from scripts import run_glm_parallel_repair as light
...
def test_roster_reaches_both_branches_without_changing_prompt_or_selector(seat, inputs):  # noqa: F811
    ...
    for old, new in ((old_graph, graph), (old_phase, phase)):
        ...
        if seat == "grok":
            ...
            assert new["reasoning"] == {"effort": "low", "exclude": True}
            assert new["provider"]["only"] == ["together"]
...
def test_glm_uses_openrouter_key_endpoint_and_native_billing(reasoning_tokens, inputs, tmp_path, monkeypatch):  # noqa: F811
    selected, _, _ = inputs
    accesses, requests = [], []
    ...
    def post(url, *, json, headers, timeout):
        ...
        response = {"model": light.MODELS["grok"], "provider": "Together",
    ...
    with light.lightweight_protocol():
        calls = light.GLMCalls(tmp_path, limits={"openrouter_usd": Decimal(1), "xai_usd": Decimal(0), "aliyun_cny": Decimal(0)},
...
@pytest.mark.parametrize("profile", (light.previous.PROFILE, light.compact.PROFILE, light.compact.LEGACY_PROFILE))
def test_old_prepared_directories_keep_original_dispatch(command, profile, tmp_path, monkeypatch):
    ...
    observed = []
    monkeypatch.setattr(light.previous, "dispatch", lambda *args, **kwargs: observed.append(args))
...
def test_selected_roster_matches_archived_default_manifest():
    # The active default now selects the joint mainline; this test covers v1.3.0.
    path = entry.DEFAULT_MANIFEST.parent / "configs/defaults/parallel-phase-repair-v1.3.0-glm-low_before_mainline_20260911.json"
