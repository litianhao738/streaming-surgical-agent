...
def audit(output):
    ...
    for video in sorted({r["video_id"] for r in rows}):
        ...
        per_video[video] = {arm: fresh.old.compute_repair_comparison([
            {**truths[r["video_id"], r["frame_id"]], "h0": r["h0"], "h1": None, "final": r[arm]}
            for r in selected])["arms"]["final"] for arm in ("h0", *fresh.old.ARMS)}
    audit = {"independent_head_tables_verified": 15, "aggregate_metrics": aggregates, "per_video_metrics": per_video,
    ...
    print(json.dumps({k: v for k, v in audit.items() if k not in ("transport_complete_sensitivity", "invalid_reasons", "per_video_metrics")}, ensure_ascii=False))
