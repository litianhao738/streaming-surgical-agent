...
更新：2026-09-09。版本 **`parallel-phase-repair-v1.0.0-experimental`**，分支 `research/parallel-phase-repair-v1`。
...
| Grok 审核席 | `grok-4.6` | xAI 直连；low reasoning |
| Qwen 审核席 | `qwen3.8-flash` | 阿里云工作空间；关闭 thinking |
...
A、B 各调用这五个审核席一次；是 **10 次审核请求，不是同一份回答同时审核五头**。每目标在 H0 后最多 **11 次调用＝1 次补候选＋5 次四头审核＋5 次 Phase**。两路审核重叠时最多 10 个请求在途、同一家最多 2 个。本次不使用提供方 Batch API，也未更换更快的 Grok。
