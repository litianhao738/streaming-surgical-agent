"""Frozen new-target comparison: shared fresh H0/proposal/Phase, compact joint vs split."""
...
PROFILE = "new_training_compact_split_confirmation_v1"
VIDEOS = ("VID103", "VID23", "VID31", "VID96")
TASKS = ("instrument", "verb", "target", "ivt", "phase")
LIMITS = {"openrouter_usd": "3", "xai_usd": "2", "aliyun_cny": "2"}
MAX_CALLS = 176
GAP = 250
SOURCE = ROOT / "artifacts/preflight/prior_candidate_eight_20260908_v1"
RULE = ("Eight new time/mask-selected Training targets in the same four videos. Same fresh H0, proposal pool, "
...
def choose(adapter, excluded):
    ...
    for video in VIDEOS:
        ...
        for numerator in (1, 2):
            anchor = samples[len(samples) * numerator // 3].target_frame_id
            ...
            selection.append({"key": f"{video}_{s.target_frame_id}", "video_id": video, "frame_id": s.target_frame_id,
                "anchor_frame_id": anchor, "quantile": f"{numerator}/3", "causal_frame_ids": list(s.causal_frame_ids),
...
def prepare(output, adapter):
    ...
    deps = {*ROOT.glob("scripts/*.py"), *ROOT.glob("src/**/*.py"), *ROOT.glob("src/**/*.txt"),
        *ROOT.glob("src/**/*.json"), *ROOT.glob("src/**/*.csv"), ROOT / "DEFAULT_PIPELINE_VERSION.json",
        ROOT / "configs/perception/joint_openrouter_h0.yaml", ROOT / "tests/unit/test_fresh_split_review.py"}
    ...
    old.save(output / "plan.json", {"profile": PROFILE, "created_utc": old.now(), "selection": selection,
        ...
        "selection_rule": "nearest eligible 1/3 and 2/3 targets; all three image identities >250 raw frames from historical/planned identities; masks only",
...
def score(output, adapter):
    ...
    checks = {"all_targets_with_arms": len(records) == 8 and all(set(r["arms"]) == set(old.ARMS) for r in records),
