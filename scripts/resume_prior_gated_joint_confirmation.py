"""Resume the interrupted prior-gated joint-Phase confirmation without repeating any attempt.

Replays every recorded request of the interrupted archive (successes, refusals
and failures), verifies request identity byte-for-byte, and spends only on
tuples that were never dispatched. Budget file writes retry locally on the
Windows file-lock error that interrupted the source run; there are no API
retries, roster changes, threshold changes or ground-truth reads.
"""
...
from scripts import run_prior_gated_joint_confirmation as trial
from scripts.check_candidate_panel_providers import redact_images
from scripts.run_repair_revision_trial import parse_review_json  # transport parser

COPIED = ("plan.json", "offline_preflight.json", "wire_fingerprints.json")
SUCCESS = ("JSON_PARSED", "JSON_PARSED_FENCE_NORMALIZED")
...
def prepare(source, output, adapter):
    ...
    prior = trial.read(source / "priors" / f"{plan['video']}.json")
    bases = trial.collector.validation_bases(adapter, plan)
    ...
    for selected in plan["selection"]:
        key = selected["key"]
        if calls_by_target[key] != trial.CALLS_PER_TARGET:
            break
        record = trial.run_target(ledger, bases[key], selected, prior, plan["gate"])
        replayed.append({"key": key, "video_id": selected["video_id"], "frame_id": selected["frame_id"],
                         "predictions": record["predictions"]})
    if [{k: r[k] for k in ("key", "predictions")} for r in replayed] != [{k: r[k] for k in ("key", "predictions")} for r in saved]:
        raise ValueError("offline replay must exactly match every completed target")
    output.mkdir(parents=True, exist_ok=True)
    for name in COPIED:
        shutil.copyfile(source / name, output / name)
    for name in ("calls", "frozen_source", "priors"):
    ...
    shutil.copyfile(Path(__file__), output / "frozen_source/scripts/resume_prior_gated_joint_confirmation.py")
    print(json.dumps({"replayed_attempts": len(ledger.used), "saved_targets_matched": len(saved),
                      "reconstructed_targets": len(replayed), "new_calls_max": plan["max_calls"] - len(ledger.rows),
                      "api_calls": 0}), flush=True)
...
def execute(output, adapter):
    ...
    prior = trial.read(output / "priors" / f"{plan['video']}.json")
    bases = trial.collector.validation_bases(adapter, plan)
        ...
        try:
            for selected in plan["selection"]:
                if calls.stopped:
                    break
                key = selected["key"]
                record = trial.run_target(calls, bases[key], selected, prior, plan["gate"])
                ...
                print(json.dumps({"target": key, "targets_completed": len(rows), "unique_attempts": len(calls.rows),
                                  "phase": record["phase_decision"]["reason"]}), flush=True)
