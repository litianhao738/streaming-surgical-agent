"""User-selected lightweight reviewer roster for both default repair branches.
...
VERSION = "parallel-phase-repair-v1.2.0-lightweight-reviewers"
PROFILE = "fixed_h0_parallel_graph_and_phase_compact_lightweight_v1"
MODELS = {**compact.original.MODELS, "grok": "mistralai/ministral-8b-2512", "qwen": "qwen3.5-35b-a3b"}
PROVIDERS = {**compact.original.PROVIDERS, "grok": "Mistral"}
RATES = {**compact.original.RATES_V2, "grok": ("0.00000015", "0.00000015")}
FAMILIES = {"grok": "Mistral", "qwen": "Qwen", "gpt": "GPT", "gemini": "Gemini", "deepseek": "DeepSeek"}
...
TRANSPORTS = {"grok": {"endpoint": "https://openrouter.ai/api/v1", "credential_slot": "gpt",
    "account": "openrouter_usd", "provider": "mistral", "allow_fallbacks": False},
...
def lightweight_body(body, seat):
    ...
    if seat == "grok":
        ...
        result["provider"] = {"only": ["mistral"], "order": ["mistral"],
...
class LightweightCalls(_TIMED_CALLS):
    def __init__(self, *args, **kwargs):
        # The legacy seat key 'grok' now runs a non-reasoning Mistral model.
        kwargs["reasoning_seats"] = tuple(s for s in kwargs.get("reasoning_seats", ()) if s != "grok")
...
def lightweight_protocol():
    with compact.compact_protocol():
        ...
        try:
            ...
            compact.original.TimedCalls = LightweightCalls
            compact.original_score.verify_plan = verify_plan
            transport.MODELS = {**transport.MODELS, "grok": (
                TRANSPORTS["grok"]["endpoint"], MODELS["grok"], "docs/API.txt", "mistral")}
...
def finalize_plan(output):
    ...
    plan.update(profile=PROFILE, repair_version=VERSION, base_repair_version=compact.VERSION,
    ...
    plan["policy"] = (
        ...
        "User-selected roster: Mistral Ministral 3 8B, Qwen3.5-35B-A3B, GPT-5.6-luna, Gemini-3.5-flash-lite, "
        "DeepSeek-v4-flash-vision-exp. Legacy seat key grok identifies Mistral, charged to OpenRouter with its "
        "assigned key and strict Mistral route. No xAI requests. No extra rounds, GT input or new H0 calls.")
    plan["limitations"] += [
        "Roster selected by user, not established as semantic best. Prior 8-target Phase ratings trial had unchanged final predictions; no new four-head accuracy evidence.",
...
def dispatch(command, output, adapter, *, source=compact.original.SOURCE):
    ...
    if read(output / "plan.json")["profile"] != PROFILE:
        ...
        return compact.dispatch(command, output, adapter, source=source)
    with lightweight_protocol():
